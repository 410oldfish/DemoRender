...
namespace nelems
{
	...
	public:
      ...
      mFOV = fov;
		...
		void update(nshaders::Shader* shader) override
		{
			...
			shader->set_vec3(mPosition, "viewPos");
		}
    ...
    glm::mat4 get_view_projection() const
    {
      return mProjection * get_view_matrix();
		...
		void move_forward(float dir)//1 forwrad -1 back
		{
			mFocus += dir * get_forward() * cMoveSpeed_Forward;
			update_view_matrix();
		}

		void move_right(float dir)
		{
			mFocus += dir * get_right() * cMoveSpeed_Right;
			update_view_matrix();
		}
    ...
    float mFar;
		...
		const float cMoveSpeed_Forward = 0.4f;
		const float cMoveSpeed_Right = 0.4f;
	};
}
